module.exports = {
  ...
  async run(m, { sock, text }) {
    ...
    const pesanSaat = group?.welcomeMsg || "Belum diatur (menggunakan pesan default)";

    if (!text) {
      return m.reply(
        `*– 乂 Welcome Message*\n\n> *Status:* ${status ? "✅ Aktif" : "❌ Nonaktif"}\n> *Pesan:* ${pesanSaat}\n\n> Gunakan:\n> • *welcome on* → aktifkan\n> • *welcome off* → nonaktifkan\n> • *welcome set [pesan]* → atur pesan custom\n> • *welcome reset* → reset ke default\n\n📌 _Variabel: {nama} {grup} {total}_`
      );
    }
    ...
    if (sub === "on") {
      ...
      db.list().group[m.cht].welcome = true;
      m.reply(`*✅ Pesan Welcome Berhasil Diatur!*\n\n> *Preview:*\n${pesan.replace(/{nama}/g, "@" + m.sender.split("@")[0]).replace(/{grup}/g, m.metadata.subject).replace(/{total}/g, m.metadata.participants.length)}`);
    } else if (sub === "reset") {
      delete db.list().group[m.cht].welcomeMsg;
